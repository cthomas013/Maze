...
public class MazeApplication extends JFrame {
	...
	/**
	 * Constructor that loads a maze from a particular file or uses a particular algorithm to generate the maze.
	 * Also, loads a specific robot driver algorithm for the maze.
	 */
	public MazeApplication( String param1, String param2) 
	{
		if ("Prim".equalsIgnoreCase(param1)) {
			System.out.println("MazeApplication: generating random maze with Prim's algorithm");
			if ("Wallfollower".equalsIgnoreCase(param2)) {
				this.driver = new WallFollower(Order.Builder.Prim);
				controller = new MazeController(Order.Builder.Prim, this.driver);
			}
			else if ("Wizard".equalsIgnoreCase(param2)) {
				this.driver = new Wizard(Order.Builder.Prim);
				controller = new MazeController(Order.Builder.Prim, this.driver);
			}
			init();
			return;
		}
		if ("Kruskal".equalsIgnoreCase(param1)) {
			System.out.println("MazeApplication: generating random maze with Kruskal's algorithm");
			if ("Wallfollower".equalsIgnoreCase(param2)) {
				driver = new WallFollower(Order.Builder.Kruskal);
				controller = new MazeController(Order.Builder.Kruskal, this.driver);
			}
			else if ("Wizard".equalsIgnoreCase(param2)) {
				this.driver = new Wizard(Order.Builder.Kruskal);
				controller = new MazeController(Order.Builder.Kruskal, this.driver);
			}
			init();
			return;
		...
		if (f.exists() && f.canRead())
		{
			System.out.println("MazeApplication: loading maze from file: " + param1);
			controller = new MazeController(param1) ;
			if ("Wallfollower".equalsIgnoreCase(param2)) {
				driver = new WallFollower();
				controller = new MazeController(param1, this.driver) ;
			}
			init();
			return ;
		}
